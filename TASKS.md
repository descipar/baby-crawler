...
- [x] **Sprachfilter**: Setting `crawler_lang_filter_enabled` + `crawler_lang_filter_langs` (Default `de`). Anzeigen in nicht erlaubten Sprachen werden via `langdetect` anhand ausreichend langer Beschreibungen herausgefiltert. Vinted-Detailbeschreibungen werden bei neuen Treffern aus dem HTML-Kopf nachgeladen und gecacht; Produkttitel werden nie allein klassifiziert. Kurze/fehlende Beschreibungen, unsichere Erkennungen und Erkennungsfehler werden durchgelassen. Settings-Tab Crawler.
